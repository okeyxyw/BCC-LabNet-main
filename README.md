...
> 现有基于深度学习的低光照图像增强方法，其核心局限源于在标准RGB色彩空间中亮度、对比度与色彩属性的固有耦合。这种纠缠导致增强结果常出现色彩失真、噪声放大及可解释性差等问题。本文提出BCC-LabNet，一种通过深度融合CIE-Lab色彩空间的感知均匀性与Retinex理论的物理可解释性而构建的新型增强框架。本工作的核心创新在于提出了一种解耦增强范式：首先，通过一个三子网结构在Lab空间内显式、独立地完成亮度、对比度与色彩的参数化重建；进而，引入一个Lab-Retinex协同细化模块，将物理先验嵌入网络，引导照度-反射分解，从而在统一流程中实现细节增强与噪声抑制的一体化。在四个公开基准数据集上的大量实验表明，BCC-LabNet取得了最优或并列最优的性能，例如在LOLv2-Real数据集上PSNR达到28.17 dB，SSIM达到0.9321。尤为关键的是，在非配对数据上的评估以及极具挑战性的下游任务（DarkFace目标检测）实验，系统性地验证了本方法卓越的泛化能力与实际应用价值。本项工作昭示了将色彩科学与物理模型相融合，在构建可解释、强泛化的底层视觉任务解决方案方面的巨大潜力。
